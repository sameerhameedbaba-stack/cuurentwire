 ...
 * The app imports it statically, and until 2026-09-04 that import was a lie:
 ...
 * so the weekly gsc-bot commit evaluated to "skip build" and this file never
 * reached production on its own. Production served whatever copy had been
 * compiled into the last build that shipped for some OTHER reason, which made
 * the thin-story policy usually inert rather than wrong (see the fallback
 * below). Measured on real commits: 9ee4cc9, bf493b7 and 520caef all exited
 * 0 = skip.
 *
 * FIXED 2026-09-04 (backlog 00b) without dropping `:(exclude)data`, which
 * would re-create the cost blowout: scripts/vercel-ignore-build.sh now ANDs a
 * second diff over the three files under data/ that are compiled into the
 * bundle, so a commit touching only this file builds while the ~12x/day report
 * commits still skip. The same three commits exit 1 = build under the new
 * rule.
 *
 * The staleness fallback below stays load-bearing anyway — a deploy can
 * still lag a report by days — and it fails safe: a deployed report past
 * GSC_SIGNALS_MAX_AGE_DAYS counts as NO data and the policy stops noindexing
 * anything.
 *
